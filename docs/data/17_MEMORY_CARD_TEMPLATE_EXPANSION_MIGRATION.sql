...
-- UNAPPLIED: Cards final template expansion. Requires separate release approval.
-- Prerequisites: SQL15 and SQL16. Existing rows, v1-v3 readers, ownership and
-- INSERT policy semantics remain unchanged. No remote application in this task.
...
begin
  ...
  case template
    ...
    when 'instant_memory' then
      ...
      date_keys := array['trip.start','trip.end'];
      primary_key := 'main_title';
    when 'double_memory' then
      slot_ids := array['dm1','dm2'];
      text_rules := '{"main_title":[34,2],"subtitle":[54,2],"photo.dm1.caption":[42,2],"photo.dm2.caption":[42,2],"footer_note":[100,3]}'::jsonb;
      date_keys := array['trip.start','trip.end'];
      primary_key := 'main_title';
    when 'triptych_story' then
      slot_ids := array['ts1','ts2','ts3'];
      text_rules := '{"main_title":[34,2],"subtitle":[54,2],"lead_note":[80,2],"photo.ts2.title":[42,2],"photo.ts3.title":[42,2],"footer_note":[100,3]}'::jsonb;
      date_keys := array['trip.start','trip.end'];
      primary_key := 'main_title';
    when 'gallery_four' then
      slot_ids := array['gf1','gf2','gf3','gf4'];
      text_rules := '{"main_title":[34,2],"subtitle":[54,2],"photo.gf1.caption":[42,2],"photo.gf2.caption":[42,2],"photo.gf3.caption":[42,2],"photo.gf4.caption":[42,2]}'::jsonb;
      date_keys := array['trip.start','trip.end'];
      primary_key := 'main_title';
    when 'hero_mosaic' then
      slot_ids := array['hm1','hm2','hm3','hm4','hm5'];
      text_rules := '{"main_title":[34,2],"subtitle":[54,2],"lead_note":[80,2],"photo.hm2.caption":[42,2],"photo.hm3.caption":[42,2],"photo.hm4.caption":[42,2],"photo.hm5.caption":[42,2],"closing_note":[100,3]}'::jsonb;
      date_keys := array['trip.start','trip.end'];
      primary_key := 'main_title';
    else return false;
  end case;
